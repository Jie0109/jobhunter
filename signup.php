<?php
    ...
    $email = $password = $username = $utype = "";
    $email_err = $password_err = $username_err = $utypeErr = "";
    ...
    if(isset($_POST['register']))
    {
        ...
        {
            $utypeErr = "<b>Please enter option</b>";
        }
        else
        {
            $utype = $_POST["utype"];
        ...
        if(empty($email_err) && empty($password_err) && empty($username_err) && empty($utypeErr))
        {
            $hash = password_hash($password, PASSWORD_DEFAULT);
            
            $sql = "INSERT INTO users (username, email, password, pw, mode) VALUES ('$username', '$email', '$password', '$hash', 'user')";
                            ...
                            <div class="col-md-6">
                                <div class="login_page bg-white shadow rounded p-4">
                                    <div class="text-center">
                                        <h4 class="mb-4">Register <a href="comSignup.php">as an employer</a></h4>
                                    </div>
                                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="login-form">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="form-group position-relative">                                               
                                                    <label>User Name (Cannot be changed)<span class="text-danger">*</span></label>
                                                    <input type="text" class="form-control" placeholder="User Name" value="" name="uname" required="">
